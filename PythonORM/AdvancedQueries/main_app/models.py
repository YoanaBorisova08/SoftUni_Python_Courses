...
from main_app.validations import RatingValidator, ReleaseYearValidator
...
class VideoGame(models.Model):
    ...
    release_year = models.PositiveIntegerField(validators=[
        ReleaseYearValidator('The release year must be between 1990 and 2023')
    ])
    rating = models.DecimalField(max_digits=2,decimal_places=1,
                                 validators=[
                                     RatingValidator('The rating must be between 0.0 and 10.0')
                                 ])
...
class Task(models.Model):
    ...
    def completed_mid_priority_tasks(cls):
        return cls.objects.filter(is_completed=True, priority='Medium')
    ...
    def recent_completed_tasks(cls, days: int):
        return cls.objects.filter(Q(completion_date__gte=F('creation_date') - timedelta(days=days)) & Q(is_completed=True))
