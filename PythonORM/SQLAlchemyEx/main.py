...
engine = create_engine('postgresql+psycopg2://postgres:1234@localhost/sqlalechemy')
...
def get_recipes_with_chef():
    recipes_with_chefs = (session.query(Recipe.name, Chef.name)
                          .join(Chef, Recipe.chef).all())
    return '\n'.join(f'Recipe: {r_name.name} made by chef: {c_name.chef.name}' for r_name, c_name in recipes_with_chefs)
