...
def register(**user):
    ...
    if not all(pass_validation_map.values()):
        render_register_screen(error="The password must contain at least one uppercase letter, one lowercase letter, one digit and one special character!")
        return

    user.update({"products":[]})

    with open("db\\user_credentials.txt", "r+", newline="\n") as file:
